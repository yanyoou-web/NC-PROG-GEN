...
## ワーク種別レジストリ（段階移行フェーズ1〜5）

ワーク固有情報をテンプレートJSに集約するリファクタ。フェーズ1〜5まで移行済み。UI表示・加工径・標準テンプレート選択はレジストリ駆動になった。**残るのは一部の特殊処理のみ**（後段フェーズ）。
...
- **レジストリ駆動になったもの**:
  ...
  - → **新規の標準ワーク種別は、テンプレJSの `registerWorkType` と `gui-v2.html` の `<script>` 追加だけで UI 表示・生成まで通る**（`gui-v2.js`/`logic-v2.js` へ workType ごとの分岐を足す必要はない）。
- **まだレジストリ化していないもの（後段フェーズ）**: `logic-v2.js` に残る**特殊処理**（Tube のチューブデータ置換・M40 の x50u8・G12B のノーズR・M12/M12_MH のバリアント選択）は個別分岐のまま。`blocks-v2.js` のワーク固有の平底出口設定も従来どおり。これらを使うワーク種別を追加する場合のみ該当箇所の変更が要る。
- 生成される Gコードはリファクタ前と完全一致する（全ゴールデンケース＋全45種×全スタイルで出力差分ゼロを確認済み）。`npm run check:worktypes` が全ワーク種別について径マップ一致とレジストリ整合を機械的に保証する。
