...
## 3. 値カテゴリ色分け表示の設計（v1のみ・現在v2で機能停止中）

生成結果のGコードを「入力値そのまま」「自動計算値」「機械固有の定数」の3種に色分けして目視監査できた仕組み。
`docs/v1-v2-feature-gap.md` のA-5でも触れているが、再実装する場合のために設計をここに保存する。
...
v2の `logic-v2.js` は `wrapH*` を同じ回数呼んでいるため、分類情報自体は今も計算されている。
`validators-v2.js` 側の `wrapH*` を `<span>` 生成版に差し替え、CSSとチェックボックスUIを移植すれば復元できる。

---

## 4. オペレーターヒント吹き出しの設計パターン（`operator-hints.js`）
...
## 6. デバッグパネルの構成（`assets/v1/debug.js`、全519行・v2未移植）

再実装する場合の設計メモとして、6ペイン構成と役割を記録する（詳細実装は `assets/v1/debug.js` を参照。
v1削除後もgit履歴から復元可能）。
