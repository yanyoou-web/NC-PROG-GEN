...
> **v1本体は2026-07-19付で削除済み**（削除前の最終コミット: `d88c683`）。引用しているファイルパスは
> ワーキングツリーには存在しないが、git履歴からは参照可能。ここに書いた内容は削除後も記録として残す。
...
## 4. オペレーターヒント吹き出しの設計パターン（`operator-hints.js`） 【2026-07-19付でv2側は廃止確定】

**2026-07-19、v2への再実装はしない方針が確定**（`docs/v1-v2-feature-gap.md` 解決済み節参照）。v2側に残っていた死んだ文言データ
（`i18n-v2.js` の `*FocusHint` 系11キーおよび `cpUsesIdDepthHint`）も削除済み。
将来「現場向け入力ヒント」を作りたくなった場合の参考として、設計をここに保存しておく。
...
```css
.nc-input-focus-popover {
  display: none; /* ...吹き出しの見た目... */
}
.nc-input-popover-wrap:focus-within .nc-input-focus-popover {
  display: block;
}
```
...
### おまけ: スタイル説明の補足ノート文言（2026-07-19付でv2側は廃止確定）

「通常バイト加工」「ヨセ中継」カード横の※展開ノートも同日付で廃止確定（`docs/v1-v2-feature-gap.md` 参照）。
UI機能としては廃止だが、説明文自体はスタイル選択の仕様理解に役立つため文言だけ残す:

- 通常バイト加工: 「自動計算されないモードで任意深さに設定可能。同内径プログラムなどに使用してください」
- ヨセ中継: 「ヨセ中継: 1工程目内径深さの計算専用です。」
