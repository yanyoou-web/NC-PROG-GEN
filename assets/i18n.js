...
(function () {
  ...
  const T = {
    ja: {
      ...
      valAFocusHint:
        "※ 偏心モード: 軸中心から加工中心までの距離 A（横方向）を半角で入力。",
      valBFocusHint:
        "※ 偏心モード: 軸中心から加工中心までの距離 B（縦方向）を半角で入力。",
      valCornWFocusHint:
        "※ 角ありモード: 素材の幅寸法を半角 mm で入力。",
      valCornHFocusHint:
        "※ 角ありモード: 角の追加高さ寸法を半角 mm で入力。",
      drillDepthFocusHint:
        "※ ドリル深さを半角 mm で入力。平底・一文字DR スタイルでは内径深さから自動計算されるため、通常は内径深さのみ入力してください。",
      idDepthFocusHint:
        "※ 図面の内径深さ（穴の深さ）を半角 mm で入力。交差穴（加工径小）スタイルでは穴交差点の距離 IP を入力してください。",
      yoseDFocusHint:
        "※ ヨセ／ヨセ中継の相手径(Φd)。図面値を半角で入力。確定時に内径寸法との関係が不正だとブラウザの注意が出ます。",
      yoseTotalLengthFocusHint:
        "※ ヨセ中継専用: ワークの全長を半角 mm で入力。1工程目の内径深さ算出に使用します。",
      yosePartnerDepthFocusHint:
        "※ ヨセ中継専用: 2工程目の相手径深さを半角 mm で入力。全長と合わせて1工程目の内径深さを自動計算します。",
      btnMaxOdCalc: "計算",
      btnMaxOdApply: "適用",
      ...
      drillMode: "ドリル モード",
      drillZ: "ドリル深さ",
      drillAutoPlaceholder: "自動計算されるので基本入力不要",
  ...
  function t(key) {
    var oh =
      typeof window.NC_OPERATOR_HINTS === "object" &&
      window.NC_OPERATOR_HINTS !== null
