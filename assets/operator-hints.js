...
(function () {
  ...
  window.NC_OPERATOR_HINTS = {
    ...
    ateLengthFocusHint:
      "※プリセット選ぶか、半角数値で直接入力。" +
      "最大径計算モード「アテ長さ」を選んだ場合には外径最大径の自動算出に使われます。",

    /** 外径最大径 … フォーカス時の吹き出し（#maxOD） */
    maxOdFocusHint:
      "※ 図面の外径最大径を半角数値で入力。" +
      "算出する場合はこの欄をクリックして最大径 計算モードを開いてください。",

    /** 最大径計算モード「通常」母材 A */
    valStockAFocusHint:
      "※ 通常モード: 素材の外径寸法 A を半角 mm で入力。" +
      "B と合わせて √(A²+B²) で外径最大径を算出します。",

    /** 最大径計算モード「通常」母材 B */
    valStockBFocusHint:
      "※ 通常モード: 素材の外径寸法 B を半角 mm で入力。" +
      "A と合わせて √(A²+B²) で外径最大径を算出します。",
    ...
    yoseDFocusHint:
      ...
      "確定時に内径寸法との関係が不正だとブラウザの注意が出ます。",

    /** ヨセ中継 全長（#yoseTotalLength） */
    yoseTotalLengthFocusHint:
      "※ ヨセ中継専用: ワークの全長を半角 mm で入力。" +
      "1工程目の内径深さ算出に使用します。",
